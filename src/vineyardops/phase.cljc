(ns vineyardops.phase
  "Phase gate for the vineyard operations coordinator rollout (0->3
  maturity). Mirrors `cattleops.phase` (cloud-itonami-isic-0141) in shape,
  adapted to this actor's simpler op set (no separate request-level
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`vineyardops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [vineyardops.governor :as governor]))
